...
def run(args, verbose=False):
    ...
    # kaichi
    if checkattr(args, "OOD"):
        input_TF = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),])
        bs = 128

        imagenet_root = "/mnt/data/Imagenet_resize"
        imagenet_testsetout = datasets.ImageFolder(imagenet_root, transform=input_TF)
        imagenet_loader = torch.utils.data.DataLoader(imagenet_testsetout, batch_size=bs, shuffle=False, num_workers=2)

        lsun_root = "/mnt/data/LSUN_resize"
        lsun_testsetout = datasets.ImageFolder(lsun_root, transform=input_TF)
        lsun_loader = torch.utils.data.DataLoader(lsun_testsetout, batch_size=bs, shuffle=False, num_workers=2)
    ...
    if (feature_extractor is not None) and args.depth>0:
        if depth==1: # you can skip preprocessing
            if verbose:
            ...
            try:
                ...
                for idx, x in enumerate(train_datasets):
                    torch.save(x, "../data/feats/train_features{}.pt".format(idx))
                for idx, x in enumerate(test_datasets):
                    torch.save(x, "../data/feats/test_features{}.pt".format(idx))
            except:
                print("could not saved")

            if checkattr(args, "OOD"):
                imagenet_datasets = utils.preprocess(feature_extractor, imagenet_loader, config, batch=args.batch,
                                             message='<IMAGENET> ', isOOD=True)
                lsun_datasets = utils.preprocess(feature_extractor, lsun_loader, config, batch=args.batch,
                                             message='<LSUN> ', isOOD=True)
                try:
                    for idx, x in enumerate(imagenet_datasets):
                        torch.save(x, "../data/feats/imagenet_features{}.pt".format(idx))
                    for idx, x in enumerate(lsun_datasets):
                        torch.save(x, "../data/feats/lsun_features{}.pt".format(idx))
                except:
                    print("could not saved imagenet")
        
        else:
            from glob import glob
            train_files = glob(os.path.join("../data/feats/train_features*.pt")).sort()
            test_files = glob(os.path.join("../data/feats/test_features*.pt")).sort()
            imagenet_files = glob(os.path.join("../data/feats/imagenet_features*.pt")).sort()
            lsun_files = glob(os.path.join("../data/feats/lsun_features*.pt")).sort()
            for fn in train_files:
                train_datasets.append(torch.load(fn))
            for fn in test_files:
                test_datasets.append(torch.load(fn))
            for fn in imagenet_files:
                imagenet_datasets.append(torch.load(fn))
            for fn in lsun_files:
                lsun_datasets.append(torch.load(fn))
    ...
    # kaichi in-distribution
    for i in range(args.contexts):
        acc, scores, penu = evaluate.test_acc(
    ...
    #kaichi
        if i == 0:
            ...
            np_scores = np.concatenate([np_scores, scores], axis=0)
            np_penu = np.concatenate([np_penu, penu], axis=0)
    np.save("../data/{}_ID_softmax.npy".format(model.name), np_scores)
    np.save("../data/{}_ID_penultimate.npy".format(model.name), np_penu)
    ...
    # kaichi imagenet
    print("OOD detection (Imagenet)")
    for i, ds in enumerate(imagenet_datasets):
        acc, scores, penu = evaluate.test_acc(
            model, ds, verbose=False, test_size=None, context_id=i, allowed_classes=list(
                range(config['classes_per_context']*i, config['classes_per_context']*(i+1))
            ) if (args.scenario=="task" and not checkattr(args, 'singlehead')) else None,
        )
        if verbose:
        ...
        if i == 0:
            ...
            np_scores = np.concatenate([np_scores, scores], axis=0)
            np_penu = np.concatenate([np_penu, penu], axis=0)
    np.save("../data/OOD/{}_Imagenet_softmax.npy".format(model.name), np_scores)
    np.save("../data/OOD/{}_Imagenet_penultimate.npy".format(model.name), np_penu)
    
    # kaichi lsun
    print("OOD detection (LSUN)")
    for i, ds in enumerate(lsun_datasets):
        acc, scores, penu = evaluate.test_acc(
            model, ds, verbose=False, test_size=None, context_id=i, allowed_classes=list(
                range(config['classes_per_context']*i, config['classes_per_context']*(i+1))
            ) if (args.scenario=="task" and not checkattr(args, 'singlehead')) else None,
        ...
        if i == 0:
            ...
            np_scores = np.concatenate([np_scores, scores], axis=0)
            np_penu = np.concatenate([np_penu, penu], axis=0)
    np.save("../data/{}_LSUN_softmax.npy".format(model.name), np_scores)
    np.save("../data/{}_LSUN_penultimate.npy".format(model.name), np_penu)
