...
def test_acc(model, dataset, batch_size=128, test_size=1024, verbose=True, context_id=None, allowed_classes=None,
    ...
    for x, y in data_loader:
        ...
        with torch.no_grad():
            if checkattr(model, 'stream_classifier'):
                penu, scores = model.classify(x.to(device), context=context_tensor)
            else:
                penu, scores = model.classify(x.to(device), allowed_classes=allowed_classes)
            
            # kaichi
            penu = penu.cpu()
            scores = torch.nn.Softmax(dim=1)(scores).cpu()
    ...
    # kaichi
        ...
        penu_list.append(penu)
    for i, (s, p) in enumerate(zip(scores_list, penu_list)):
        if i == 0:
            np_scores = s.to("cpu").detach().numpy().copy()
            np_penu = p.to("cpu").detach().numpy().copy()
        else:
            np_scores = np.concatenate([np_scores, s], axis=0)
            np_penu = np.concatenate([np_penu, p], axis=0)
    ...
    return accuracy, np_scores, np_penu # (1000), (1000, 100), (1000, 2000)
